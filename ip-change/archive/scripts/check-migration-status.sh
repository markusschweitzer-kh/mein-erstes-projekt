...
check_host() {
    ...
    # Prüfe DNS-Server (wenn SSH verfügbar)
    if command -v ssh >/dev/null 2>&1 && [ -n "$reachable_ip" ]; then
        dns_servers=$(ssh -o ConnectTimeout="$TIMEOUT" -o StrictHostKeyChecking=no "$reachable_ip" "cat /etc/resolv.conf 2>/dev/null | grep -E '^nameserver' | awk '{print \$2}' | tr '\n' ' '" 2>/dev/null || echo "")
        if [ -n "$dns_servers" ]; then
            # Prüfe ob neue DNS-Server (9.0.0.1 und 9.0.0.2) konfiguriert sind
            if echo "$dns_servers" | grep -q "9.0.0.1" && echo "$dns_servers" | grep -q "9.0.0.2"; then
                status_dns="NEU"
    ...
    if [ "$status_9x" = "NEU" ] && [ "$status_10x" = "NEU" ] && [ "$status_hostname" = "NEU" ] && [ "$status_dns" = "NEU" ]; then
        overall_status="OK"
        symbol="✓"
        color="$GREEN"
        HOSTS_OK+=("$hostname_neu|$ip_neu|$ip_10_neu|$status_9x|$status_10x|$status_hostname|$status_dns")
    elif [ "$status_9x" = "ALT" ] && [ "$status_10x" = "ALT" ] && [ "$status_hostname" = "ALT" ] && [ "$status_dns" = "ALT" ]; then
        overall_status="NICHT MIGRIERT"
        symbol="○"
        color="$YELLOW"
        HOSTS_FAILED+=("$hostname_alt|$ip_alt|$ip_10_alt|$status_9x|$status_10x|$status_hostname|$status_dns")
    else
        overall_status="TEILWEISE"
        symbol="◐"
        color="$MAGENTA"
        HOSTS_PARTIAL+=("$hostname_alt/$hostname_neu|$reachable_ip|$ip_10_alt/$ip_10_neu|$status_9x|$status_10x|$status_hostname|$status_dns")
    fi
...
if [ $ok_count -gt 0 ]; then
    ...
    for host in "${HOSTS_OK[@]}"; do
        IFS='|' read -r hostname ip_9x ip_10x status_9x status_10x status_hostname status_dns <<< "$host"
...
if [ $partial_count -gt 0 ] || [ $failed_count -gt 0 ]; then
    ...
    if [ $partial_count -gt 0 ]; then
        ...
        echo "  1. Prüfe welche Komponente fehlt (9.x IP, 10.x IP, Hostname oder DNS)"
        echo "  2. Verwende fix-9x-ip-manual.sh für 9.x IP-Probleme"
        echo "  3. Verwende change-10x-ip.sh für 10.x IP-Probleme"
        echo "  4. Verwende update-dns-config.yml für DNS-Probleme"
        echo "  5. Führe network-update-rhel-v3.yml erneut aus"
        echo ""
    fi
